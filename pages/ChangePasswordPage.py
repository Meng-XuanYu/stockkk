    ...
    def change_password(self):
        ...
        if result == QMessageBox.Yes:
            ...
            if not current_password or not new_password or not confirm_new_password:
                ...
                information_box.setText("修改密码失败，所有字段均不能为空")
                information_box.setWindowTitle("失败")
                information_box.setStandardButtons(QMessageBox.Ok)
                information_box.exec_()
            elif not re.match(
                    r'^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&#])[A-Za-z\d@$!%*?&#]{8,20}$', new_password):
                information_box = QMessageBox()
                information_box.setIcon(QMessageBox.Information)
                icon_pixmap = QPixmap('images/images/stockkk.jpg').scaled(64, 64)
                information_box.setIconPixmap(icon_pixmap)
                information_box.setText("修改密码失败，新密码不符合要求")
                information_box.setWindowTitle("失败")
                information_box.setStandardButtons(QMessageBox.Ok)
                information_box.exec_()
                return
                ...
                icon_pixmap = QPixmap('images/images/stockkk.jpg').scaled(64, 64)
                information_box.setIconPixmap(icon_pixmap)
                information_box.setText("修改密码失败，当前密码错误")
                information_box.setWindowTitle("失败")
                information_box.setStandardButtons(QMessageBox.Ok)
                information_box.exec_()
            else:
                if new_password == confirm_new_password:
                    ...
                    icon_pixmap = QPixmap('images/images/stockkk.jpg').scaled(64, 64)
                    information_box.setIconPixmap(icon_pixmap)
                    information_box.setText("修改密码成功，跳回桌面")
                    ...
                    icon_pixmap = QPixmap('images/images/stockkk.jpg').scaled(64, 64)
                    information_box.setIconPixmap(icon_pixmap)
                    information_box.setText("修改密码失败，新密码不匹配")
                    information_box.setWindowTitle("失败")
                    information_box.setStandardButtons(QMessageBox.Ok)
                    information_box.exec_()
